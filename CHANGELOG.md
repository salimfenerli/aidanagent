# CLAUDE.md'den taşınan seans günlükleri (24 Tem 2026 arşivlendi)
# Aşağıdaki 7–21 Temmuz paketleri deploy edildi; CLAUDE.md kısaltıldı, detay burada.

### 21 Temmuz 2026 — 3. paket: 💪 HEVY ENTEGRASYONU (v7-111) — HENÜZ DEPLOY EDİLMEDİ
Salim: "hevy uygulamam var bunu aidana bağlayabilir miyiz · bağlı olsun her şeye, gelişime baksın".

- **⚠️ ÖN KOŞUL: Hevy Pro.** API anahtarı `hevy.com/settings?developer` adresinden alınır, **ücretsiz hesapta üretilemez** (401/403). Bağlanamazsa UI net Türkçe hata gösterir ve anahtarı KAYDETMEZ (yanlış anahtar sessizce durmasın).
- **API:** `https://api.hevyapp.com/v1/workouts`, `api-key` header, `pageSize` MAX 10. Şema Hevy'nin kendi `hevyapp/hevy-gpt` reposundaki OpenAPI spec'inden **doğrulandı** (tahmin değil): tüm alanlar snake_case — `start_time`, `weight_kg`, `reps`, `exercise_template_id`, set `type` ∈ warmup|normal|failure|dropset.
- **Anahtar nerede:** `data.settings.hevyKey` (Supabase RLS + kendi cihazı). Tarayıcı Hevy'yi doğrudan çağıramaz (CORS) → **worker proxy** `POST /hevy-sync {key}`. CSP değişikliği GEREKMEDİ, worker zaten `connect-src`'de.
- **`normalizeHevyWorkout`** → kompakt `{id, date, title, durationMin, volumeKg, setCount, exercises:[{name,tid,sets,volumeKg,top:{kg,reps,e1rm}}]}`. **TR gün kayması** `start_time`'a TR ofseti eklenerek çözülür (22:30Z antrenmanı ertesi güne yazılır). Savunmacı parse: eksik/null alan, agırlıksız kardiyo seti, bozuk tarih → çökmez.
- **Gelişim metriği = tahmini 1RM (Epley** `kg × (1 + tekrar/30)`**)** — 60kg×8 ile 70kg×5'i kıyaslayabilmek için tek sayıya indirger. **Isınma setleri rekora GİRMEZ** (`type==='warmup'` filtresi); hacme girer.
- **Rekor push'u:** `computeHevyPRs` + `newHevyPRs(old,new,since)` — sadece **son 3 günün** rekorları kutlanır, yoksa ilk senkronda tüm geçmiş bildirim olarak patlar. `🏆 Bench Press: 65kg × 5`.
- **Senkron:** (1) akşam cron'unda **plandan ÖNCE** (`runHevySync` → `runAutoPlan`) — bugünün antrenmanı plan geçmişine işlensin, (2) uygulama açılışında 6 saatte bir sessiz, (3) Ayarlar'dan "Şimdi çek". Manuel test `?type=hevy`.
- **🧠 Planlayıcıya bağlandı (asıl kazanç):** `planHistory[].w` = antrenman günü etiketi. `planProfile` artık **antrenman günü vs normal gün** tamamlama oranını ayrı ölçüyor; fark ≥%15 ise prompt'a "antrenman günlerinde oran düşüyor, daha AZ ve KISA blok koy" kuralı giriyor (her iki tarafta da ≥4 blok şartı). Ayrıca `gymDayLine()`: **Hevy sadece GEÇMİŞİ verir, gelecek programı bilmez** → "son 3 haftanın 2+'sinde bu gün antrenman yapıldı" desenine bakıp "bugün muhtemelen antrenman günü, ~90 dk boşluk bırak" diyor.
- **UI:** Ayarlar → Hevy bağlantısı (anahtar/test/kes + durum satırı). Diyet sekmesi altında **gelişim paneli**: bu hafta · son 4 hafta · toplam hacim · önceki 4 haftaya göre trend %, son antrenman, en sık 4 egzersizde 1RM değişimi + rekorlar. Skor kartına **antrenman hücresi** (sadece Hevy bağlıysa görünür).
- **Yeni veri:** `data.hevy = {workouts[], prs{}, lastSync, lastError}` (son 180 gün, id bazlı dedupe), `settings.hevyKey`, `planHistory[].w`
- **Doğrulama:** 33 senaryo daha (**toplam 119**): gerçek Hevy şemasıyla parse, TR gece yarısı kayması, ısınma seti filtresi, e1RM matematiği, eksik alan dayanıklılığı, dedupe/birleştirme/eskitme, rekor tespiti + tekrar kutlamama, antrenman günü profil hesabı. + `node --check` + HTML denge.
- ⚠️ **Canlı test YAPILAMADI** — sandbox'ta Hevy erişimi ve anahtar yok. İlk "Bağlan" denemesi gerçek testtir.
- Cache v7-110 → **v7-111**.

### 21 Temmuz 2026 — 2. paket: 🧠 PLANLAMA ZEKASI (v7-110) — HENÜZ DEPLOY EDİLMEDİ
Salim: "kendisi planlama yapabilsin zeki şekilde". Tespit: planlayıcı her gün SIFIRDAN başlıyordu — Salim'in gerçek hızını, hangi saatte blok tutturduğunu, neyi ertelediğini bilmiyordu. **Kök sorun: geçmiş planlar hiçbir yerde saklanmıyordu** (`dayPlan` her gün üzerine yazılıyor) → öğrenecek veri yok. 4 parça:

- **📦 `data.planHistory[]` (YENİ VERİ, son 21 gün):** `dayPlan` üzerine yazılmadan önce `archivePlanDay()` ile arşivlenir. Kompakt format `{date, blocks:[{s,e,k,d,c}]}` (start/end/kind/done/kategori). Kategori arşiv anında linkli görevden çözülür (görev sonra silinse bile veri kalır). Tarih bazlı dedupe. Çağrı yerleri: worker `runAutoPlanForUser` (üzerine yazmadan önce) + frontend `renderDayPlan` (gün değişiminde) + `planMyDay`.
- **📊 `planProfile()` — ölçülmüş gerçek (worker + tasks.js ikizi):** (1) **saat dilimi tutturma oranı** sabah<12:00 / öğle 12–17 / akşam 17+ (mola ve sabit bloklar sayılmaz — onlar "tutturma" değil), (2) **süre sapması** = `actualMin/estimateMin` **MEDYANI** son 30 biten görevden (ortalama tek uç değerle bozuluyordu; 0.5–3 arası kırpılır, <3 görevde `null`), (3) **günlük gerçekçi kapasite** (ortalama biten blok), (4) **en çok kaçan kategori**. 3 günden az veride sessiz kalır — AI'ya uydurma istatistik gitmez.
- **🎯 Profil → plana nasıl giriyor:** süreler AI'ya gönderilmeden ÖNCE sapma katsayısıyla çarpılır (5 dk'ya yuvarlı) — AI'ya "şu kadar uzat" demek yerine sayı zaten düzeltilmiş gider, prompt "tekrar uzatma" der. `profileLines()` prompt'a Türkçe kurallar ekler: düşük oranlı saate ağır iş koyma · max N çalışma bloğu (ortalama+1) · en çok kaçan kategoriyi en iyi saate ve KISA bloklar halinde koy · güne 20-30 dk tampon bırak.
- **⏳ Deadline/sınav farkındalığı:** `deadlineLines()` — `data.countdowns`'tan 14 güne kadar olan sınav/teslimler prompt'a girer. Ayrıca **çok günlük iş güne bölünür**: `due` + `estimateMin` varsa `bugün = tahmin / (kalan gün + 1)` (min 20 dk), AI'ya "bugün ~45dk (toplam 180dk, son tarihe bölündü)" olarak gider — tek oturuşta bitirmeye çalışmaz.
- **🔁 Otomatik toparlama (worker, butonsuz):** `shiftPlanBlocks()` = frontend `shiftPlanRemaining`'in worker ikizi. 5 dk cron'da 20+ dk gecikmiş blok varsa plan kendiliğinden yeniden dizilir + "🔁 Plan yeniden dizildi · Sıradaki: X" push'u. **Günde max 3 kez** (`dayPlan.autoShifts`, plan sürekli oynamasın), `settings.autoShift===false` ile kapatılır. **Ping'lerden ÖNCE çalışır** (yoksa az sonra kayacak blok için "şimdi bunu yap" denir) ve o turda ping atmaz — aynı anda iki bildirim yerine tek net mesaj.
- **▶️ "Şu an çalışılan blok yerinde kalır"** — hem worker hem frontend kaydırmada `start <= now < end` olan bitmemiş blok sabit sayılır, cursor onun bitişinden başlar. Tam ortasında çalışırken bloğun yerini değiştirmek en can sıkıcı davranıştı.
- **👁️ Şeffaflık satırı:** plan panelinde `#planInsight` — "Son 12 günün verisiyle planlanıyor · süreler 1.4× · günde ~4 blok · en iyi: öğleden sonra". 3 günden az veride "Aidan planlarını izliyor — 1/3 gün" der. Salim neden öyle planlandığını görsün.
- **Yeni veri:** `data.planHistory[]`, `settings.autoShift`, `dayPlan.autoShifts`
- **Doğrulama:** 38 senaryo daha (toplam **86**): arşiv dedupe/kırpma, bucket oranları, mola sayılmaması, medyan sapma + uç değer kırpma, az-veri sessizliği, sapma×gün-bölme birlikte, otomatik shift tetikleme/limit/kapalı, çalışan blok korunması, idempotentlik (aynı planı tekrar dizince değişmemeli), ay aşımı tarih hesabı. + tüm dosyalarda `node --check` + HTML tag dengesi.
- Cache v7-109 → **v7-110**.

### 21 Temmuz 2026 — 🗓️ OTOMATİK GÜN PLANI PAKETİ (v7-109) — HENÜZ DEPLOY EDİLMEDİ
Salim: "günümü düzenlemede sorun yaşıyorum, her şeyi planlasın ve mesaj atarak hatırlatsın". Tespit: `planMyDay` (AI gün planı) ve push cron'u vardı ama **birbirine bağlı değildi** — plan yapılıyor, kimse haber vermiyordu. 5 parçalı paket:

- **♻️ `generatePlanBlocks(env,{tasks,from,to,now,busy})`** — plan üretimi `handlePlanApi`'den çıkarıldı, artık hem `/plan` endpoint'i hem cron'lar kullanıyor. `busy` = sabit program aralıkları, prompt'a "bu saatlere ASLA blok koyma" olarak geçer.
- **🌙 Akşam yarını planlar (varsayılan):** `0 18 * * *` (21:00 TR) akşam özetinden sonra `runAutoPlan(env,{trigger:'evening',forDate:trDate(1)})` → **yarının** planı hazır, gece düzeltilebilir. `data.settings.planWhen = 'evening'|'morning'` (varsayılan evening). Sabah `0 5 * * *` **güvenlik ağı**: o tarihe plan yoksa bugünü planlar. Atlama kuralları: `autoPlan===false` → kapalı · evening trigger + morning modu → atla · `dp.date===forDate && blocks.length` → **elle/önceden yapılmış plana DOKUNMA**.
- **📚 Haftalık sabit program:** `data.fixedSchedule=[{id,label,days:[0..6 JS getDay],start,end,enabled}]` (okul/özel ders/antrenman). `fixedBlocksFor(data,dateStr)` (worker) + `fixedBlocksForDate(dateStr)` (tasks.js) ikizi. Sabit bloklar plana ÖNCE konur, AI talimata rağmen üstüne blok koyarsa `blocksOverlap` ile **atılır** — sabit program her zaman kazanır. Ayarlar'da CRUD (gün chip'leri, aç/kapa, sil). Bu olmadan otomatik plan okul saatlerine görev koyuyordu.
- **⏱️ Blok bildirimleri:** `runPlanBlockPings` — blok saatinde "▶️ Şimdi: X · 16:00–16:45 · 45 dk" push'u, `b.pinged` ile blok başına tek sefer, 10 dk gecikme toleransı. Cron **`*/15` → `*/5`** (15 dk'da bir yarım saatlik bloğun ortasına düşüyordu); sabit hatırlatıcılar aynı cron'da, `lastFired` guard'ı sayesinde çift atmıyor (test edildi).
- **🔔 Bildirimden direkt aksiyon:** push payload'a `actions:[Başlat|Bitti|15dk ertele]` + `data.blockId`; `sendPushToAll` artık `tag`/`url`/`actions`/`blockId` geçiriyor (blok başına ayrı tag → bildirimler birbirini ezmiyor). `sw.js notificationclick`: aksiyon varsa `e.action`, **iOS PWA aksiyon butonu GÖSTERMEZ** → gövdeye tıklanır, `act='open'` gelir. Uygulama açıksa `postMessage({type:'BLOCK_ACTION'})`, kapalıysa `/?blk=..&act=..`. Frontend `initBlockActionBridge()` ikisini de yakalar.
- **🔁 Plan toparlama (`shiftPlanRemaining`)** — ADHD'de planın ölme sebebi "bir blok kaçtı, gerisi çöp". Gecikmiş blok varsa gün planının tepesinde uyarı + "Yeniden diz". **Kural tabanlı, AI YOK, anında.** Biten + sabit bloklar yerinde kalır, hareketli bloklar şu andan itibaren yeniden dizilir. **Boşluk doldurmalı:** sabit bloktan önce 40 dk boşluk varsa ve sıradaki blok sığmıyorsa sığan bir sonraki blok oraya alınır (sıra mümkün olduğunca korunur, boşluk ziyan olmaz). Sığmayan düşer. `snoozeBlock(id,15)` kaskadlı erteleme.
- **Pencere kalıcı:** `savePlanWindow` artık `settings.planFrom/planTo`'ya da yazıyor (dayPlan her gün sıfırlanıyordu, worker okuyamıyordu). Varsayılan okul-duyarlı: hafta içi 16:00–22:00, hafta sonu 10:00–22:00.
- **`renderDayPlan` yarın desteği:** dayPlan bugün VE yarın dışındaki tarihlerde sıfırlanır; yarınsa "şu an" vurgusu ve gecikme uyarısı kapalı, alt başlık değişir.
- **Manuel test:** `?type=autoplan` · `?type=autoplan-tomorrow` · `?type=planpings`
- **Yeni veri:** `data.fixedSchedule[]`, `settings.planWhen|autoPlan|planPings|planFrom|planTo`, `dayPlan.auto`, `dayPlan.blocks[].pinged`, `dayPlan.blocks[].fixedId`
- **Doğrulama:** 48 senaryo node testi (blok ping zamanlama, sabit program gün eşleşmesi, çakışma, boşluk doldurma, pencere taşması, autoplan atlama kuralları, tarih sınırları) + tüm dosyalarda `node --check`. ⚠️ **Canlı test yapılmadı** — deploy sonrası ilk gece/sabah izlenecek.
- Cache v7-106 → **v7-109**.

### 11 Temmuz 2026 — 2. seans: temel analiz paneli + portföy geneli risk (v7-106)
- **📊 Temel analiz paneli (feature 1 — backend):** app şimdiye kadar %100 teknikti; Salim temel+teknik çalıştığı için temel veri eklendi. **Worker `/stock-fundamentals`** (`handleStockFundamentalsApi`): Yahoo `quoteSummary` proxy — **crumb+cookie akışı** (`getYahooCrumb`: fc.yahoo.com cookie → v1/test/getcrumb → v10 quoteSummary, isolate'te 30dk cache; 401/403'te crumb sıfırlanır). Modüller: summaryDetail+defaultKeyStatistics+financialData+price. Döner: F/K, ileri F/K, PD/DD, temettü verimi, piyasa değeri, EPS, net kâr marjı, ROE, borç/özsermaye, ciro/kâr büyümesi, analist hedefi+görüşü. **Frontend:** grafik modalına 3. sekme **"Temel"** (`stockFundTabBtn`, sadece BIST/ABD'de görünür — `loadStockHistory` taOk ile toggle), `loadStockFundamentals`+`renderStockFundamentals` mevcut `.stock-ta-grid`/`.stock-ta-cell` stilini yeniden kullanır (yeni CSS yok). ⚠️ **BIST kapsamı eksik olabilir** (F/K, PD/DD, piyasa değeri genelde var; ROE/marj/büyüme Türk hisselerinde boş "—" gelebilir) — canlı test gerek. Crumb akışı sandbox'tan test edilemedi (Yahoo erişimi yok), worker'da çalışır.
- **🛡️ Portföy geneli risk paneli (feature 2 — frontend):** tek-hisse risk panelinin portföy versiyonu. `renderPortfolioRisk()` (stocks.js, `renderStocks`'ta çağrılır, `#portfolioRiskPanel` portföy özeti altında): **tüm stop'lar tetiklenirse toplam kayıp** (para birimi başına, stop = `w.alarmBelow` varsa o, yoksa %8 varsayılan), **en yoğun pozisyon** (kendi para biriminde ağırlık, >%25 amber uyarı), **en çok riske açık** pozisyon, **çeşitlendirme** (pozisyon+piyasa sayısı, <3 uyarı). FX-free (para birimleri ayrı, kur karıştırılmaz), mutasyonsuz. CSS `.pfr-*`.
- **🧹 ⏳ temizliği:** stocks.js kalan 3 ⏳ loading-prefix'i kaldırıldı.
- **⚠️ Worker deploy:** `.github/workflows/deploy.yml` `aidan-worker/worker.js` değişince worker'ı da deploy ediyor (GitHub secret'larıyla) — bu commit worker'ı canlıya alır.
- Cache v7-105 → **v7-106**.

### 11 Temmuz 2026 — borsa risk/stop-loss önerisi + emoji sweep tamamlandı
- **📉 Stop-loss önerisi (borsa seviye atlama):** Salim "risk yönetimi + stop öneri" istedi. Grafik modalı TA panelinin altına `#stockRiskPanel` — mevcut TA'dan (ATR + destek/direnç) **kural tabanlı** stop + hedef türetir (AI YOK, anında). `taStopSuggestion(ta,w)` (stocks.js): ATR stop = `fiyat − 2×ATR`; destek stop = `20-periyot destek × 0.99`; **öneri seçimi** = destek makul mesafedeyse (risk ≤%12) yapı-tabanlı, değilse ATR, o da yoksa sabit %8. Çıktı: stop, %risk, hedef (2:1 R:R), en yakın dirence göre gerçekçi R:R (dar ise amber uyarı), pozisyon varsa **TL risk** (para birimi duyarlı). `renderStockRisk` render eder, `setStopAlarm(stop)` önerilen stop'u `w.alarmBelow`'a yazar → **mevcut cron push** kırılınca haber verir (yeni backend YOK). CSS `.rk-*` (styles.css, Impeccable: tam kenar tint, amber buton, danger/success değerler). Doğrulama: 4 senaryo node math testi + assertion'lar geçti.
- **🧹 Emoji sweep (v60 politikası yeni modüllere de uygulandı):** diyet/borsa/classroom modülleri v60'tan sonra geldiği için dekoratif emoji geri gelmişti. Kalan buton-glyph'leri Lucide SVG'ye (takvim/odak/bitir/sil/bağlan/link/kopyala/portföy-ekle), mit-check ✅/⬜ → check-square/square SVG, welcome-logo ☁️ → cloud SVG. Toast/status ✅❌ prefix + 👆👇🎯📊✂️🔔⭐🟠🎙️📧🔄 kaldırıldı, `fx` piyasa rozeti 💱 → `$`. **Korunan:** ✓✕★ mono + durum noktaları 🟢🔴🟡 + bayraklar 🇹🇷🇺🇸 + ₿ + kod yorumlarındaki emoji.
- **📐 Pozisyon boyutu (risk-tabanlı, v7-105):** Salim "framework'ün 3. adımını app yapsın" dedi. `riskPositionSize(price,stop,cur)` (stocks.js): sermaye = **aynı para birimindeki** portföy değeri (`riskPortfolioValue`, nakit yok — Salim bilerek seçti), `adet = (sermaye × risk%) ÷ (giriş − stop)`. Çıktı: kaç lot, TL risk, pozisyon değeri, portföy ağırlığı %; ağırlık >%25 → konsantrasyon uyarısı. Risk% chip'leri (1/1.5/2/3, `data.settings.riskPct`, varsayılan 1.5) `setRiskPct` ile canlı yeniden çizer. Risk paneline eklendi, CSS `.rk-size/.rk-chip`. Doğrulama: node testi (para birimi izolasyonu + risk≤bütçe assertion) geçti.
- Cache v7-102 → **v7-103** (emoji) → **v7-104** (risk paneli) → **v7-105** (pozisyon boyutu).
