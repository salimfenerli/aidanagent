...
**Ölçüm yöntemi 5 Eylül'ün aynısı** (`/tmp/diag.js` kalıbı): 8 profil × 20 kombinasyon (4 gün tipi × 5 şablon), fikir yürüterek değil ÜRETİLEN PLANDAN. Baseline `git show HEAD:nutrition.js` ile yan yana koşuldu.

**🔴 1 — PROTEİN ÖĞÜNLERE EŞİT BÖLÜNÜYORDU, ANA ÖĞÜN ÇAPASI YARIMLANIYORDU.** 5 Eylül'de çapa tabanını 1 porsiyona çekmek denenmiş, gün proteini 20 kombinasyonun 11'inde sert tavana yapıştığı için **geri alınmıştı**; o günün notu kök nedeni doğru teşhis etmişti: *"nutMealSplit proteini beş öğüne EŞİT bölüyor, ana öğün payı ara öğünle aynı."* Şimdi kök neden düzeltildi.
- `NUT_PROTEIN_RATIO` eklendi. **Oranlar sezgiden değil GERÇEK PORSİYONDAN türetildi:** Türk ana yemeği tek porsiyonda 38-47 g protein taşıyor (somon 40, tavuk göğsü 47, levrek 38). Ana öğün payı bunun altında kalırsa motorun tek çıkışı çapayı yarımlamaktır.
- Kural bilerek **asimetrik**: taban SERT (hiçbir öğün 0.25 g/kg altına itilmez — eşik altı öğün MPS'i maksimuma çıkarmaz, kayıp gerçek), tavan YUMUŞAK (0.40 g/kg üstünün ek anabolik faydası yok ama zararı da yok). Yani ana öğünü banda sığdırmak için ara öğünü eşiğin altına itmek yanlış takas olurdu.
- **"Taban + kalanı oranla" denendi ve BIRAKILDI:** oranları sulandırıyordu (69 kg'da tabanlar 85 g'ı peşin dağıtıyor, oranlanacak 53 g kalıyor, ana öğün yine 31 g'da kalıyordu). Doğrusu önce ORANI uygulamak, tabanı sonra KIRPMA olarak dayatmak.
- `anaTaban` kancası artık boş değil: ana öğünde çapa tabanı 1 porsiyon.

**🔴 2 — ARA ÖĞÜN HAVUZUNDA HAFİF SEÇENEK YOKTU** (20 Ağu notunun *"çözüm oran değil ŞABLON HAVUZU"* tespiti). Havuzun beşinin de çapası protein yoğundu (protein tozu 24 g, süzme yoğurt 16 g/kase) ve birim tabanı 1 — ara öğün HEDEFİ 18 g olsa bile 24 g teslim ediyordu. Günün protein bütçesi orada tükendiği için ana öğün tam porsiyon çapayı hiç alamıyordu. İki hafif şablon eklendi. ⚠️ İlkinde çapa `Kefir` yazıldı ve **motorun kendi sözleşme testi kırmızıya döndü** (0.06 g protein/kcal): hafif olsun derken *seyreltik* çapa seçmek "4 bardak kefir" hatasını geri getirir. Aranan şey seyreltik değil **küçük ama yoğun** çapa — `Cottage peyniri` 11 g'ı 98 kcal'de taşıyor (0.112) ve birimi porsiyon olduğu için yarım adımlarla oynuyor.

**🔴 3 — ŞABLON SEÇİMİ KÖR ROTASYONDU.** Rotasyon hâlâ birincil sıra ("Başka öner" çalışsın diye) ama öğünün protein hedefini kabaca ıskalayan aday artık atlanıyor; hiçbiri tutmazsa **en yakını** seçiliyor (plan üretilmeden kalmaz). ⚠️ Toleransı **asimetrik yapmak denendi ve ölçüm reddetti**: gerekçe makuldü (ana öğünde fazla protein sorun değil) ama amacı olan şeyi — havuz çeşitliliğini — geri getirmedi ("Tavuk göğsü + bulgur" 39 g hedefe 70 g teslim ediyor, her makul tavanı aşıyor) ve gün proteini her profilde 1.5-3.5 puan kötüleşti. Simetrik %35 kaldı.

**🔴 4 — YAĞ 20 GÜNÜN 16'SINDA HEDEFİN ALTINDA BİTİYORDU** (5 Eyl'in "kalan bilinen zayıflığı", ort. -%5,2, enerji payı zaman zaman %24 — DRI ergen AMDR tabanı %25). Tavan gevşetilmedi; ana öğüne tam porsiyon çapa gelince **gıdanın kendi yağı** yükseldi ve açık kendiliğinden kapandı.

| ölçüm (8 profil × 20 kombinasyon) | önce | sonra |
|---|---|---|
| yağ hedefin %3'ünden fazla altında | **85 / 160** | **24 / 160** |
| yağ enerji payı %25 altında (DRI tabanı) | **48 / 160** | **6 / 160** |
| kalori hedefin %3'ünden fazla altında | 19 / 160 | 13 / 160 |
| ana öğünde yarım porsiyon çapa | her gün | **hiç** |

⚠️ **BEDELİ VAR, GİZLENMİYOR:** gün proteini 2-4 puan yükseldi (69 kg/kas: +%9,5 → +%11). Motorun kendi doktrini bunu kaldırıyor (2.5 g/kg azalan getiri noktasıdır, güvenlik sınırı değil) ama plan "hedef 138 g" yazıp 153 g veriyor — `nutDaySummary` bunu zaten yazıyor, saklanmıyor. Hafif profillerde karbonhidrat -%3..-%9'a düştü; 45 kg/koru en kötü köşe (c -%8,8) ve **açık bilinen sınır** olarak buraya yazılıyor.

**🥗 KİŞİSEL TERCİH KATMANI (yeni).** Plan zaten *"sevmediğini benzer makrolu başka yiyecekle değiştir"* diye not yazıyordu — yani işi kullanıcıya devrediyordu. Somon sevmeyen biri o notu okuyup her gün kafadan takas yapmaz, planı bırakır. Takas artık motorun işi.
- `data.diet.nut.tercih = { diyet, sevmem[], favori[] }`. Tek dokunuşla üç durum: nötr → ★ favori → ✕ sevmem. (İki ayrı liste ekranı bilerek yapılmadı: telefonda 50 kalemlik listede mod değiştirmek kaybolmanın en hızlı yolu.)
- Çapa engellenirse şablon ATILMAZ, **çapa takas edilir** (aynı slotun havuzundan) — "somon sevmiyorum" diyen birinin akşam öğünü kaybolmamalı, sadece balığı değişmeli.
- **KAÇIŞ KAPISI:** bir slotun bütün şablonları takılırsa öğün kaybolmaz; kural o tek öğün için gevşetilir ve **arayüzde yazılır**. Sessizce 4 öğünlük gün üretmek, makro toplamını da öğün sayısını da yanlış yapar ve kullanıcı fark etmez.
- ⚠️ **VEJETARYEN SEÇENEĞİ BİLEREK YOK.** Eklendi, ölçüldü: öğle+akşam havuzunda balıksız+etsiz hiçbir çapa kalmıyor, kaçış kapısı her gün iki öğünde birden açılıyordu. Baklagil çapası da çözmüyor — motorun kendi yoğunluk sözleşmesi ≥0.08 g protein/kcal istiyor, kuru fasulye 0.056, mercimek yemeği 0.060; hedefe ölçeklenince "4 porsiyon kuru fasulye" üretirdi. **Arayüzde görünen ama tutmayan seçenek, olmayan seçenekten kötüdür** — teste bağlandı.
- ⚠️ **BU BİR ALERJİ ÖZELLİĞİ DEĞİL** ve arayüzde "alerji" kelimesi geçmiyor: motor yalnız kendi şablon havuzundaki ~50 adı tanır, günlüğe elle yazılanı ya da paketli ürünün içindekiler listesini görmez.

**🍽️ "NE YESEM?" — KALAN MAKROYU KAPATAN ÖNERİ (yeni).** Günlük zaten *"32 g protein, 240 kcal kaldı"* diyordu ama ne yiyeceğini söylemiyordu; o boşluk kullanıcının üstünde kalıyor ve günün sonunda kapanmıyordu — motorun kendi "asıl risk AZ YEMEK" ilkesine ters. Adaylar: kendi kaydettiği besinler → sık yedikleri → tohum tablo. **Ağ çağrısı ve AI yok**, deterministik.
- ⚠️ İlk ölçümde saf makro puanı **"3 porsiyon Adana kebap (1440 kcal)"** üretti: matematik doğru, öneri uygulanamaz. Tek öneri ≤700 kcal, büyük birimde ≤2 porsiyon.
- ⚠️ Saf makro puanı her seferinde tohum tabloyu seçiyordu; kullanıcının kendi kaydettiği "Annemin köftesi" hiçbir listede görünmüyordu. Kaynak çarpanı eklendi (kendi 1.35 · sık yediği 1.2 · tablo 1) — **sıralamayı** etkiler, makro doğruluğunu ezmez.
- ⚠️ Günün kalorisinin %60'ından azında makro bilgisi varsa P/K/Y açığı UYDURMADIR: sıralama yalnız kaloriye düşer ve bu **ekranda yazılır**. O durumda beraberliği protein yoğunluğu bozar (ilk üçe "2 porsiyon Profiterol" giriyordu).
- Geçmişten gelen kalemin birimi yok — "2 porsiyon Yumurta" değil **"2× Yumurta"** yazılır; çarpan kaydın kendisine uygulanır.

**Regresyon:** `tests/36-beslenme-tercih.test.js` (18 test) — kısıt yüzünden öğün kaybolmuyor · sunulan her diyet kısıtı kaçış kapısı açmadan uygulanabiliyor · tercih yokken filtre hiçbir şeyi elemiyor · öneri porsiyon/kalori tavanı · ana öğünde yarım porsiyon çapa yok. `18-nutrition`'daki **"protein öğünlere EŞİT dağıtılıyor" testi tersine çevrildi** — o sözleşme motorun hatasıydı.

⚠️ **İLK YÜKLEME BÜTÇESİ DUVARA DAYANDI: 184,7 / 185 KB.** Yeni arayüz `nutrition.js` (tembel) içinden çiziliyor ve mevcut sınıflar (`dt-row`, `diet-card`) kullanıldığı için CSS neredeyse büyümedi. **Bir sonraki diyet özelliği eşiği yükselterek geçirilemez** — borç ödenmeli (aday: `styles.css`'teki beslenme/karne blokları tembel bir stil dosyasına).
...
**Denetimin ONAYLADIKLARI** (değiştirilmedi): Schofield 10-18 katsayısı · 18 altında Mifflin yerine Schofield (ergen sporcuda test edilen 8 denklemden kabul edilebilir tek denklem) · protein 1.8/2.0 g/kg ve öğün başı 0.25-0.40 · ~~proteinin öğünlere EŞİT dağıtılması~~ **(9 Eyl 2026'da DEĞİŞTİ — aşağıya bak)** · antrenman günü karbonhidrat kaydırma · Ca 1300 / Fe 11 / D 600 · Fe ÜS 45, D ÜS 4000 · **kan değeri olmadan asla demir takviyesi** (denetim "motorun en iyi kuralı" dedi) · kreatin duruşu · mikro besinlerin kapsam yüzdesiyle verilmesi · kilo verme dalının hiç olmaması.



⚠️ **EŞİT DAĞITIM MADDESİ 9 EYLÜL 2026'DA GERİ ALINDI.** Denetimin gerekçesi (ISSN #6, öğün büyüklüğüne orantılı dağıtım daha kötü olur) yanlış değildi ama eksikti: eşit dağıtım öğün başı EŞİĞİ korur, ana öğünün GERÇEK PORSİYONUNU koruyamaz. 69 kg'da 138 g / 5 öğün = 27,6 g, Türk ana yemeği porsiyonu ise 38-47 g — motorun tek çıkışı çapayı yarımlamak oluyordu ("yarım porsiyon somon + 3 patates"). Yeni kural eşit dağıtım DEĞİL, orantılı dağıtım da DEĞİL: **taban korunarak ana öğüne ağırlıklı** (taban sert 0.25 g/kg, tavan yumuşak). ISSN'in koruduğu şey — hiçbir öğünün eşiğin altına düşmemesi — testle kilitli.
