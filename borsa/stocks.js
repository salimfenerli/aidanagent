// ============ BORSA MOTORU ============
// 14 Agu 2026: Aidan'dan AYRILDI — bu dosya artik kendi sitesinde yasiyor.
// Aidan cekirdeginden kullandigi 11 fonksiyon shared.js'e, kimlik/senkron
// sync.js'e, render sirasi app.js'e tasindi. Motorun kendisi (izleme listesi,
// portfoy, teknik analiz, Buffett skoru, tarama, islem gunlugu) DEGISMEDI —
// aylardir teste bagli davranis oldugu gibi korundu.
// ⚠️ Satir sonu: borsa/ klasorunde TUM dosyalar LF.
// ============ BORSA ============
      ...
      <div class="stock-card ${cardDir}" onclick="openStockChart(${idx})" title="Grafiği aç">
        ...
        <div class="stock-card-bottom">
          ...
          <div class="stock-actions" onclick="event.stopPropagation()">
            <button class="pos" onclick="setPosition('${jsArg(w.symbol)}')" title="Pozisyon (adet + maliyet)"><svg class="icon" viewBox="0 0 24 24" aria-hidden="true"><rect x="2" y="7" width="20" height="14" rx="2"/><path d="M16 7V5a2 2 0 0 0-2-2h-4a2 2 0 0 0-2 2v2"/></svg>${w.qty != null && w.qty > 0 ? 'Pozisyon' : 'Pozisyon ekle'}</button>
            <button onclick="setStockAlarm('${jsArg(w.symbol)}')" title="Alarm kur"><svg class="icon" viewBox="0 0 24 24" aria-hidden="true"><path d="M6 8a6 6 0 0 1 12 0c0 7 3 9 3 9H3s3-2 3-9"/><path d="M10.3 21a1.94 1.94 0 0 0 3.4 0"/></svg>Alarm</button>
            <button class="del" onclick="removeStock('${jsArg(w.symbol)}')" title="Listeden çıkar"><svg class="icon" viewBox="0 0 24 24" aria-hidden="true"><path d="M3 6h18"/><path d="M8 6V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2"/><path d="M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6"/></svg></button>
          </div>
        </div>
      </div>
    `;
...
function openTradeModal() {
  ...
  let msg = '';
  // NOT: Aidan'da burada bir de "dun kotu uyudun" disiplin uyarisi vardi.
  // Uyku verisi Aidan'da kaldigi icin bu kural borsa sitesinde HIC tetiklenemezdi;
  // olu dal birakmak yerine kaldirildi. (Geri istenirse uyku verisi buraya tasinmali.)
...
async function aiPortfolioTechnical() {
  ...
  btn.disabled = true;
  resEl.style.display = 'block';
  resEl.textContent = 'Portföyün teknik durumuna bakılıyor…';
  try {
    const token = window._supa ? (await window._supa.auth.getSession()).data.session?.access_token : null;
    if (!token) throw new Error('giriş yapılmamış');
  ...
  try {
    ...
    for (let idx = 0; idx < files.length; idx++) {
      setPfImportStatus(files.length > 1
        ? `Görseller okunuyor… (${idx + 1}/${files.length}) — 10-15 sn/görsel, sabret`
        : 'Görsel okunuyor… 10-15 sn sürebilir, sabret');
...
async function aiScreenComment() {
  ...
  if (box) { box.style.display = 'block'; box.innerHTML = '<div class="scr-loading">Tablo okunuyor…</div>'; }
  try {
    const token = await getSupaToken();
    if (!token) throw new Error('giriş yapılmamış');
...
function renderScreenComment() {
  ...
  box.innerHTML = `<div class="scr-comment-head">AI okuması</div>
...
function screenRowHtml(r, i) {
  ...
  return `<div class="scr-row">
    ...
    <div class="scr-row-foot">${bfHtml}${mosHtml}${kindHtml}
      <button type="button" class="scr-add" onclick="screenAddToWatchlist('${jsArg(r.symbol || '')}')">Listeme ekle</button>
    </div>
    ${weak}${cyFlags}
  </div>`;
}
...
function renderScreener() {
  ...
  el.innerHTML = `
    ...
    <button type="button" class="scr-ai" id="scrAiBtn" onclick="aiScreenComment()">Bu tabloyu anlat</button>
